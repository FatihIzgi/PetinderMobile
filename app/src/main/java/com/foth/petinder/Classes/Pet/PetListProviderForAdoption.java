...
public class PetListProviderForAdoption {
    ...
    public static ArrayList<Pet> getTheChosenAdoptionPetList(String option){
    ...
    public static Pet getTheCurrentPetForAdoption(String option, int counter){
        if(counter>=0 && counter<getTheChosenAdoptionPetList(option).size()){
            return getTheChosenAdoptionPetList(option).get(counter);
        }
        else {
            return null;
        }
